...
>(
  (
    ...
    return (
      <div
        ref={ref}
        className={cn("group text-card-foreground", "transition-all duration-300 ease-in-out", state === 'expanded' ? 'w-[var(--sidebar-width)]' : 'w-[var(--sidebar-width-icon)]')}
        data-state={state}
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex items-center h-14 p-4 lg:h-[60px] lg:px-6", className)}
      {...props}
    />
  )
})
